 ...
 */
@Getter
public class BreweryIngredients {
     ...
     * Load from File
     */
    public BreweryIngredients(List<Ingredient> ingredients, int cookedTime) {
     ...
     */
    public BreweryIngredients(List<Ingredient> ingredients, int cookedTime, boolean legacy) {
    ...
    public ItemStack cook(int state, Player brewer) {
        ...
        BreweryRecipe cookRecipe = getCookRecipe();
        Brew brew;
        ...
        if (cookRecipe != null) {
            ...
            int alcohol = Math.round(cookRecipe.getAlcohol() * ((float) quality / 10.0f));
            Logging.debugLog("cooked potion has Quality: " + quality + ", Alc: " + alcohol);
            ...
            if (state <= 0) {
                ...
                BreweryCauldronRecipe cauldronRecipe = getCauldronRecipe();
                if (cauldronRecipe != null) {
                    ...
                    if (cauldronRecipe.getCustomModelData() != 0) {
                        potionMeta.setCustomModelData(cauldronRecipe.getCustomModelData());
                    }
                }
            }
        }
        ...
        potionMeta.displayName(BreweryUtil.component("&f" + cookedName));
     ...
     */
    public @Nullable BreweryRecipe getBestRecipe(BarrelWoodType wood, float time, boolean distilled) {
    ...
    public BestRecipeResult getBestRecipeFull(BarrelWoodType wood, float time, boolean distilled) {
        if (BreweryRecipe.getAllRecipes().isEmpty()) {
     ...
     */
    public @Nullable BreweryRecipe getCookRecipe() {
     ...
     */
    @Nullable
    public BreweryCauldronRecipe getCauldronRecipe() {
        BreweryCauldronRecipe best = null;
        float bestMatch = 0;
        float match;
        for (BreweryCauldronRecipe recipe : BreweryCauldronRecipe.getAllRecipes()) {
     ...
     */
    public @Nullable BreweryRecipe getDistillRecipe(BarrelWoodType wood, float time) {
     ...
     */
    public @Nullable BreweryRecipe getAgeRecipe(BarrelWoodType wood, float time, boolean distilled) {
     ...
     */
    public int getIngredientQuality(BreweryRecipe recipe) {
    ...
    public RecipeEvaluation getIngredientQualityFull(BreweryRecipe recipe) {
        ...
        for (Ingredient ingredient : ingredients) {
            ...
            if (amountInRecipe == 0) {
                ...
                evaluation.deduct(new BrewDefect.WrongCount(ingredient, amountInRecipe), ingredientCountDeduction);
            }
        }
        return evaluation;
    }
     ...
     */
    public int getCookingQuality(BreweryRecipe recipe, boolean distilled) {
        return Math.round(getCookingQualityFull(recipe, distilled).getQuality());
    }

    public RecipeEvaluation getCookingQualityFull(BreweryRecipe recipe, boolean distilled) {
        RecipeEvaluation evaluation = new RecipeEvaluation();
        ...
        if (cookedTime < 1) {
            evaluation.deduct(new BrewDefect.CookTimeMismatch(0, recipe.getCookingTime()), 10);
            ...
            evaluation.deduct(new BrewDefect.CookTimeMismatch(cookedTime, recipe.getCookingTime()), cookTimeDeduction);
        }
        return evaluation;
    }
     ...
     */
    public int getDistillQuality(BreweryRecipe recipe, byte distillRuns) {
     ...
     */
    public int getWoodQuality(BreweryRecipe recipe, BarrelWoodType wood) {
        return Math.round(getWoodQualityFull(recipe, wood).getQuality());
    }

    public RecipeEvaluation getWoodQualityFull(BreweryRecipe recipe, BarrelWoodType wood) {
        RecipeEvaluation evaluation = new RecipeEvaluation();
        ...
        if (wood != recipe.getWood()) {
            ...
            evaluation.deduct(new BrewDefect.WrongWood(wood, recipe.getWood()), woodDeduction);
        }
        return evaluation;
    }
    ...
    private int getWoodQualityNew(BreweryRecipe recipe, BarrelWoodType wood) {
     ...
     */
    public int getAgeQuality(BreweryRecipe recipe, float time) {
        return Math.round(getAgeQualityFull(recipe, time).getQuality());
    }

    public RecipeEvaluation getAgeQualityFull(BreweryRecipe recipe, float time) {
        RecipeEvaluation evaluation = new RecipeEvaluation();
        if (!BreweryUtil.isClose(time, recipe.getAge())) {
            float ageDeduction = Math.abs(time - recipe.getAge()) * ((float) recipe.getDifficulty() / 2);
            evaluation.deduct(new BrewDefect.AgeMismatch(time, recipe.getAge(), recipe.isAlcoholic()), ageDeduction);
        }
        return evaluation;
    }
    ...
    public BreweryIngredients copy() {
        BreweryIngredients copy = new BreweryIngredients();
    ...
    public static BreweryIngredients load(DataInputStream in, short dataVersion) throws IOException {
        ...
        for (; size > 0; size--) {
            ...
            if (!PLUGIN.getIngredientLoaders().containsKey(itemLoader.saveId())) {
                Logging.errorLog("Ingredient Loader not found: " + itemLoader.saveId());
                break;
            }
            Ingredient loaded = PLUGIN.getIngredientLoaders().get(itemLoader.saveId()).apply(itemLoader);
        ...
        return new BreweryIngredients(ing, cookedTime);
    }
