...
public final class BreweryUtil {
    ...
    public static List<String> colorArrayList(List<String> list) {
        ...
        return list.stream().map(BreweryUtil::color).toList();
    }

    public static List<String> colorArray(String... list) {
        if (list == null) return null;
        return Stream.of(list).map(BreweryUtil::color).toList();
    }
    ...
    public static boolean blockDestroy(Block block, Player player, BarrelDestroyEvent.Reason reason) {
        ...
        if (type == Material.CAULDRON || type == MaterialUtil.WATER_CAULDRON) {
            ...
            BreweryBarrel breweryBarrel = BreweryBarrel.getBySpigot(block);
            if (breweryBarrel != null) {
                if (breweryBarrel.hasPermsDestroy(player, block, reason)) {
                    breweryBarrel.remove(null, player, true);
                    return true;
                } else {
                    return false;
                }
            ...
            BreweryBarrel breweryBarrel2 = BreweryBarrel.getBySpigot(block);
            if (breweryBarrel2 != null) {
                if (!breweryBarrel2.isLarge()) {
                    if (breweryBarrel2.hasPermsDestroy(player, block, reason)) {
                        breweryBarrel2.remove(null, player, true);
                        return true;
                    } else {
                        return false;
                    }
                } else {
                    breweryBarrel2.destroySign();
                }
            }
            return true;

        } else if (BarrelAsset.isBarrelAsset(BarrelAsset.PLANKS, type) || BarrelAsset.isBarrelAsset(BarrelAsset.STAIRS, type)) {
            BreweryBarrel breweryBarrel3 = BreweryBarrel.getByWood(block);
            if (breweryBarrel3 != null) {
                if (breweryBarrel3.hasPermsDestroy(player, block, reason)) {
                    breweryBarrel3.remove(block, player, true);
                } else {
                    return false;
                }
            }
